...
const MagneticSocialLink = ({ children, link }) => {
    return (
        <Magnetic springOptions={{ bounce: 0 }} intensity={0.3}>
            <a
                href={link}
                target='blank'
                className="group relative inline-flex shrink-0 items-center gap-[1px] rounded-full bg-zinc-100 px-2.5 py-1 text-xl text-black transition-colors duration-200 hover:bg-zinc-950 hover:text-zinc-50 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700">
                {children}
                <svg
                    width="15"
                    height="15"
            ...
            <div className="flex items-center justify-start space-x-3">
                {SOCIAL_LINKS.map((social) => (
                    <MagneticSocialLink key={social.label} link={social.link}>
                        <span className='me-2'>{social.label}</span>
                    </MagneticSocialLink>
                ))}
            </div>
        </div>
